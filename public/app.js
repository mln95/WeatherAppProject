...
window.onload = function () {
  ...
  async function getData() {
    ...
    function displayUI(dataWeather) {
      document.getElementById("date").textContent = dataWeather.date;
      document.getElementById("tempFahrenheit").textContent =
        Math.round(dataWeather.temp) + "°F";
      document.getElementById("tempCelsius").textContent =
        Math.round(((dataWeather.temp - 32) * 5) / 9) + "°C";
      document.getElementById("content").textContent = dataWeather.feelings;
    }
    receiveData();
  }
};
